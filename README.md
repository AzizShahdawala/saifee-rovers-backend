...
Email delivery uses Brevo SMTP through Nodemailer. Configure `SMTP_HOST=smtp-relay.brevo.com`, `SMTP_PORT=587`, `SMTP_SECURE=false`, and set `SMTP_USER` and `SMTP_PASSWORD` to the SMTP login and SMTP key shown in Brevo's SMTP & API settings. Verify `webdevelopment5253@gmail.com` as a Brevo sender and set `EMAIL_FROM=Saifee Rovers <webdevelopment5253@gmail.com>`. Never commit the SMTP key. In local development without credentials, Nodemailer's stream transport writes the generated email to the backend terminal for testing; production refuses to generate an OTP when delivery is unavailable.
